...
It allows a server to store information about a client. It can be used to persist authentication information so credentials don't need to be re-entered at every client request to the server.
...
It does multiple rounds of hashing that would require attackers to take on considerable time, energy and hardware to decrypt.

1. What are the three parts of the JSON Web Token?

Header (algorithm with the token type), payload (info like the username), signature (which includes the secret)
